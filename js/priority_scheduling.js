function priority_scheduling () {
	...
	function populateProcessList () {
		for (i = 0; i < MAX_NUMBER_OF_PROCESSES; i++) {
			...
			priority = document.getElementById('priority_' + i);
			if (time_taken != null) {
				var time_taken_value = time_taken.value;
				var processName = i;
				var arrival = document.getElementById('arrival_' + i).value;
				var priority_value = parseInt(priority.value);
				processDict = {'name': 'p' + processName, 'arrival': arrival, 'process_time': time_taken_value, 'y0': null, 'y1': null, "priority": priority_value};
